...
class GeneralHelper
{
    ...
    public static function checkRestAPI(): bool
    {
        ...
        if (!$response || isset($response->error) || (isset($response->data->status) && $response->data->status == 401) || (isset($response->code) && ($response->code == 'rest_forbidden' || $response->code == 'rest_no_route'))) {
            return false;
        }
        return true;
    }
